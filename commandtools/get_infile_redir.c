...
/*   Updated: 2023/10/31 15:32:24 by lmohin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

t_bool	check_operator_after_infile_redir(t_vars *v, char next_char);
...
char	*get_infile_redir(t_vars *v, char *line, size_t *l_index)
{
	...
	if (check_operator_after_infile_redir(v, line[*l_index + 1]))
		return (NULL);
	j = 1;
		...
		|| line[*l_index + j] == '<' || line[*l_index + j] == '>')
	{
		...
		ft_putstr_fd(" syntax error: missing infile after \"<\"\n", 2);
		v->exitstatus = 2;
		return (NULL);
	}
	(*l_index) += 1;
