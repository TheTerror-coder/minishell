...
t_bool	ft_inredir(t_vars *v, char *infile)
{
	...
	nada = __CLOSED_FD;
	if (!infile)
		return (ft_leave(v, EXIT_FAILURE, "ambiguous redirect", __PRINT));
	if (!ft_fclose(v, &v->infd))
		return (__FALSE);
	v->infd = open(infile, O_RDONLY);
	if (v->infd < 0)
		return (ft_leave(v, EXIT_FAILURE, infile, __PERROR));
	if (!ft_ioset_op(v, &v->infd, &nada))
		return (__FALSE);
	return (__TRUE);
}
...
t_bool	ft_launch_heredoc(t_vars *v, char *limiter)
{
	...
	if (!v->limiter)
		return (ft_leave(v, EXIT_FAILURE, \
				"ft_launch_heredoc(): ft_strdup() failed", __PRINT));
	if (!ft_heredoc(v))
		return (__FALSE);
	if (v->hdoc_fd < 0)
		return (ft_leave(v, EXIT_FAILURE, \
...
/*
* The heredoc file descriptor is duplicated here because otherwise, 
* when there are several heredocs, this function is called as many times
* to ultimately save the last as standard input. So if we close this hdoc_fd,
* we loose the connection with the here-document content at the first call.
*/
t_bool	ft_heredocredir(t_vars *v, t_commands *command)
{
	...
	if (hdoc_fd_dup < 0)
		return (ft_leave(v, EXIT_FAILURE, \
				"ft_heredocredir(): dup() failed", __PRINT));
	if (!ft_ioset_op(v, &hdoc_fd_dup, &nada))
		return (__FALSE);
	return (__TRUE);
}
...
t_bool	ft_outredir(t_vars *v, char *outfile)
{
	...
	if (!outfile)
		return (ft_leave(v, EXIT_FAILURE, "ambiguous redirect", __PRINT));
	if (!ft_fclose(v, &v->outfd))
		return (__FALSE);
	v->outfd = open(outfile, O_CREAT | O_WRONLY | O_TRUNC, \
					S_IRUSR | S_IRGRP | S_IROTH | S_IWUSR | S_IWGRP);
	if (v->outfd == -1)
		return (ft_leave(v, EXIT_FAILURE, outfile, __PERROR));
	if (!ft_ioset_op(v, &nada, &v->outfd))
		return (__FALSE);
	return (__TRUE);
}
